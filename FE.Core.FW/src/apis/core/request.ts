...
function getUrl(options: ApiRequestOptions): string {
    ...
    const url = options.path.includes('http') ? options.path : `${OpenAPI.BASE}${path}`;
    if (options.query) {
...
async function getHeaders(options: ApiRequestOptions): Promise<Headers> {
    ...
    const APIKEY = await resolve(options, OpenAPI.APIKEY);
    ...
    if (response.status !== 204) {
        try {
            ...
            if (contentType) {
                ...
                const isBlob = contentType.toLowerCase().startsWith('application/octet-stream');
                if (isJSON) {
