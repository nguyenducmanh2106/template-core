import { PaymentDetail, PaymentResponse } from "@/apis/models/PaymentModel";
import { getPaymentDetail } from "@/apis/services/PaymentService";
...
const Detail: React.FC<{
  ...
  useEffect(() => {
    if (props.open && props.PaymentDetail.paymentStatus != undefined) {
      ...
      getPaymentDetail(props.PaymentDetail.paymentRequest?.id ?? '')
  ...
  return (
    ...
    >
      ...
      <Row>
        ...
        <Col span={20}>{moment(props.PaymentDetail.paymentRequest?.dob, "YYYY-MM-DD").format("DD/MM/YYYY")}</Col>
      </Row>
      <Row>
        <Col span={4}>Khu vực thi</Col>
        <Col span={20}>{props.PaymentDetail.paymentRequest?.examAreaName}</Col>
      </Row>
      <Row>
        <Col span={4}>Địa điểm thi</Col>
        <Col span={20}>{props.PaymentDetail.paymentRequest?.examLocation}</Col>
      </Row>
      <Row>
        <Col span={4}>Địa chỉ</Col>
        <Col span={20}>{props.PaymentDetail.paymentRequest?.examAddress}</Col>
      </Row>
      <Row>
