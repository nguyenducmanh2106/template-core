import { Code, SuppliesCategoryModel } from '@/apis';
import { deleteSuppliesCategory, getSuppliesCategory, postSuppliesCategory, putSuppliesCategory } from '@/apis/services/SuppliesCategoryService';
import Permission from '@/components/Permission';
import CreateForm from '@/components/SuppliesCategory/CreateForm';
import UpdateForm from '@/components/SuppliesCategory/UpdateForm';
import { layoutCode, PermissionAction, suppliesSerialStatusConstant } from '@/utils/constants';
...
interface EntityFromDb {
  ...
  suppliesSerialStatus: number | undefined;
...
function App() {
  ...
  const [suppliesCategoryList, setsuppliesCategoryList] = useState<EntityFromDb[]>([]);
  ...
  const [updateModel, setUpdateModel] = useState<Partial<SuppliesCategoryModel>>({});
  ...
  const getData = async (page: number): Promise<void> => {
    ...
    const response: ResponseData = await getSuppliesCategory(searchParams.name);
    setsuppliesCategoryList((response.data || []) as EntityFromDb[]);
    setPagination({
      ...pagination,
      current: page,
  ...
  const submitCreateForm = async (model: Omit<SuppliesCategoryModel, 'id'>): Promise<void> => {
    let responseData = await postSuppliesCategory('', model);
  ...
  const submitUpdateForm = async (model: SuppliesCategoryModel): Promise<void> => {
    let responseData = await putSuppliesCategory('', model);
    if (responseData.code != 200) {
      message.error(responseData.message);
      return;
    }
  ...
  const columns: ColumnsType<EntityFromDb> = [
    ...
    {
      title: 'Vật tư có serial',
      dataIndex: ['suppliesSerialStatus'],
      render: (_, record) => <>{suppliesSerialStatusConstant.find(item => item.key == record.suppliesSerialStatus)?.value}</>
    },
    {
      title: 'Trạng thái',
      dataIndex: ['isActive'],
    ...
    },
    {
      ...
      render: (_, record) =>
        <Space>
          <Permission navigation={layoutCode.suppliesCategory} bitPermission={PermissionAction.Edit} noNode={<></>}>
            <Button type='link' onClick={() => {
              setOpenUpdateForm(true);
              setUpdateModel({ ...updateModel, ...record });
            }}>Sửa</Button>
          </Permission>
          <Permission navigation={layoutCode.suppliesCategory} bitPermission={PermissionAction.Delete} noNode={<></>}>
            <Button type='link' onClick={async () => {
              Modal.confirm({
                ...
                onOk: async () => {
                  setLoadingDataState(true);
                  try {
                    let responseData: ResponseData = await deleteSuppliesCategory('', [record.id]);
  ...
  return (
    <div className='layout-main-conent'>
      <Form form={searchForm} name='search' onFinish={searchFormSubmit}>
        <Row gutter={20}>
          <Col span={6}>
            <Form.Item label='Tên danh mục vật tư' name='name'>
              <Input placeholder='Nhập danh mục vật tư' />
            </Form.Item>
          </Col>
          <Col>
      ...
      <Divider orientation="left">
        <Space>
          <Permission navigation={layoutCode.suppliesCategory} bitPermission={PermissionAction.Add} noNode={<></>}>
              ...
              if (selectedRowDeleteKeys.length == 0) {
                ...
                Modal.confirm({
                  ...
                  onOk: async () => {
                    setLoadingDataState(true);
                    try {
                      let responseData: ResponseData = await deleteSuppliesCategory('', selectedRowDeleteKeys);
                      if (responseData.code != Code._200)
                        message.error(responseData.message);
                      else {
                        getData(pagination.current);
      ...
      <Table
        rowKey='id'
        columns={columns}
        dataSource={suppliesCategoryList}
