...
const EditExamLocationRoomInfor: React.FC<Props> = (({
    handleOk,
    recordEdit,
}: Props) => {
    ...
    return (
        <Form
            ...
            initialValues={{
                ["ExamLocationName"]: recordEdit?.examLocationName,
                ["Code"]: recordEdit?.code,
                ["Name"]: recordEdit?.name,
                ["Order"]: recordEdit?.order,
                ["Amount"]: recordEdit?.amount,
                ["Status"]: recordEdit?.status,
            }}
        >
            <Row gutter={16} justify='start'>
                ...
                <Col span={12}>
                    <Form.Item label={'Địa điểm thi'} labelCol={{ span: 24 }} wrapperCol={{ span: 24 }}
                        name='ExamLocationName' rules={[{ required: true }]}
...
export default EditExamLocationRoomInfor