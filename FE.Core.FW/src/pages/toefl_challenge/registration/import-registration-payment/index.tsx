...
function ImportRegistrationPaymentTFC({ open, setOpen, reload }: Props) {
    ...
    const handleOk = async () => {
        ...
        const response = await postRegistrationPayment1({ IsOverwrite: isOverwrite, File: file });
        if (response.code === Code._200) {
            message.success(response.message || "Ypload file thành công")
            setOpen(false);
    ...
    const downloadFileTemplate = async () => {
        ...
        const response = await getFile(TemplateType._1);
        if (response.code === Code._200) {
    ...
    return (
        <>
            <Modal title="Upload danh sách nộp lệ phí" open={open} cancelText="Bỏ qua" width={'800px'}
            ...
            >
                ...
                >
                    <Row gutter={16} justify='start'>
                        <Col span={24}>
                            <Form.Item label={'Ghi đè dữ liệu'} labelAlign='left' labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} name='IsOverwrite'>
                                <Switch />
                            </Form.Item>
                        </Col>
                        <Col span={24}>
...
export default ImportRegistrationPaymentTFC;
