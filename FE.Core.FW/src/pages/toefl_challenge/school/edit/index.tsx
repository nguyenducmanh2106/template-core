...
import { getDivision, putDivision } from '@/apis/services/toefl-challenge/DivisionService';
import { getSchool, getSchool1, putSchool } from '@/apis/services/toefl-challenge/SchoolService';
import {
...
function EditSchoolTFC() {
    ...
    useEffect(() => {
        const fnGetInitState = async () => {
            setLoading(true);
            const responseSchool: ResponseData = await getSchool1(params.id);
            const responseData: SchoolModel = responseSchool.data as SchoolModel;
            const filter = {
                provinceId: responseData.provinceId,
                departmentId: responseData.departmentId
            }
            const filterSchool = {
                provinceId: responseData.provinceId ? responseData.provinceId : undefined,
            }
            const responseSchoolParent: ResponseData = await getSchool(
                JSON.stringify(filterSchool)
            );
            const schoolOptions = ConvertOptionSelectModel(responseSchoolParent.data as OptionModel[]);
            const responseDivision: ResponseData = await getDivision(JSON.stringify(filter));
            ...
            const divisionOptions = ConvertOptionSelectModel(responseDivision.data as OptionModel[]);
    ...
    const handleOk = async () => {
        ...
        const response = await putSchool(params.id, objBody);
        setButtonOkText('Lưu');
                ...
                title={
                    <>
                        <Space className="title">
                            ...
                            <Text strong>Cập nhật thông tin trường học</Text>
                        </Space>
                    </>
                }
                extra={
            ...
            >
                {loading ? <Spin /> :
                    <Form
                        ...
                        initialValues={{
                            ["Code"]: state.schoolEdit?.code ?? '',
                            ["Name"]: state.schoolEdit?.name ?? '',
                            ["ProvinceId"]: state.schoolEdit?.provinceId ?? '',
                            ["DistrictId"]: state.schoolEdit?.districtId ?? '',
                            ["DepartmentId"]: state.schoolEdit?.departmentId ?? '',
                            ["SchoolType"]: state.schoolEdit?.schoolType ?? '',
                            ["InstitutionType"]: state.schoolEdit?.institutionType ?? '',
                            ["DivisionId"]: state.schoolEdit?.divisionId ?? '',
                            ["ParentName"]: state.schoolEdit?.parentName ?? '',
                            ["Description"]: state.schoolEdit?.description ?? '',
                        }}
                    >
                            ...
                            <Col span={12}>
                                ...
                                >
                                    <Select
                                        ...
                                        placeholder='-Chọn-' options={state.divisions} />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item
