...
import { PaymentApDetail, PaymentApSearchModel } from "@/apis/models/PaymentApModel";
import { getExamPeriodAp } from "@/apis/services/ExamPeriodApService";
import { exportPaymentHistory, searchPayment } from "@/apis/services/PaymentApService";
import Detail from "@/components/PaymentAp/Detail";
...
function App() {
  ...
  const [paymentDetail, setPaymentDetail] = useState<Partial<PaymentApDetail>>({});
  const [list, setList] = useState<PaymentApDetail[]>([]);
  const [searchParams, setSearchParams] = useState<Partial<PaymentApSearchModel>>({ pageNumber: 1, pageSize: 10 });
  ...
  const getList = async (current: number, pageSize: number = 10): Promise<void> => {
    ...
    setList((response.data || []) as PaymentApDetail[]);
  ...
  useEffect(() => { getExamPeriodAp().then(response => setListExamPeriod(response.data as ExamPeriodResponse[])) }, []);
  ...
  const columns: ColumnsType<PaymentApDetail> = [
    {
      title: "STT",
      dataIndex: "index",
      width: 80,
    ...
    },
    {
      ...
      dataIndex: "PayDate",
      render: (_, record) => <>{moment(record.paymentRequest.dateCreateRecord, "YYYYMMDDHHmmss").format("DD/MM/YYYY HH:mm:ss")}</>
    },
    {
      title: "Tổng tiền",
            ...
            <Space>
              ...
              <Button key="export" type="primary" htmlType="button" loading={loadingExport} icon={<DownloadOutlined />}
                onClick={event => {
                  ...
                  var searchObj: Partial<PaymentApSearchModel> = {
