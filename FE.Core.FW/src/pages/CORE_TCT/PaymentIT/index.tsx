...
import { PaymentITDetail, PaymentITSearchModel } from "@/apis/models/PaymentITModel";
import { getExamPeriod } from "@/apis/services/ExamPeriodService";
import { exportPaymentHistory, searchPayment } from "@/apis/services/PaymentItService";
import Detail from "@/components/PaymentIT/Detail";
...
function App() {
  ...
  const [paymentDetail, setPaymentDetail] = useState<Partial<PaymentITDetail>>({});
  const [list, setList] = useState<PaymentITDetail[]>([]);
  const [searchParams, setSearchParams] = useState<Partial<PaymentITSearchModel>>({ pageNumber: 1, pageSize: 10 });
  ...
  const getList = async (current: number, pageSize: number = 10): Promise<void> => {
    ...
    setList((response.data || []) as PaymentITDetail[]);
  ...
  const columns: ColumnsType<PaymentITDetail> = [
    {
      title: "STT",
      dataIndex: "index",
      width: 80,
    ...
    },
    {
      ...
      dataIndex: "PayDate",
      render: (_, record) => <>{moment(record.paymentRequest.dateCreateRecord).format("DD/MM/YYYY HH:mm:ss")}</>
    },
    {
      title: "Tổng tiền",
      dataIndex: ["paymentRequest", "Amount"],
      render: (_, record) => <>{ConvertIntToCurrencyFormat(Number(record.paymentRequest.amount))}</>
    },
    {
      title: "Email",
      dataIndex: "UserEmail",
    ...
    },
    {
      title: "Thao tác",
      dataIndex: "action",
      render: (_, record) => record.paymentStatus != 1 ? <Button type="link" onClick={() => { setOpenDetail(true); setPaymentDetail(record); }}>Chi tiết</Button> : <></>
    }
  ];
            ...
            <Space>
              ...
              <Button key="export" type="primary" htmlType="button" loading={loadingExport} icon={<DownloadOutlined />}
                onClick={event => {
                  ...
                  var searchObj: Partial<PaymentITSearchModel> = {
