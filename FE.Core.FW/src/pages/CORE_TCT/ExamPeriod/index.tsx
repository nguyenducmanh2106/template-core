import { Code, ExamPeriodModel } from '@/apis';
import { ExamPeriodResponse } from '@/apis/models/data';
import { deleteExamPeriod, getExamPeriod, postExamPeriod, putExamPeriod } from '@/apis/services/ExamPeriodService';
import CreateForm from '@/components/ExamPeriod/CreateForm';
import UpdateForm from '@/components/ExamPeriod/UpdateForm';
...
function App() {
  ...
  const [examPeriodList, setExamPeriodList] = useState<ExamPeriodResponse[]>([]);
  ...
  const [updateModel, setUpdateModel] = useState<Partial<ExamPeriodModel>>({});
  ...
  const getData = async (page: number): Promise<void> => {
    ...
    const response: ResponseData = await getExamPeriod(searchParams.name, searchParams.status);
    setExamPeriodList((response.data || []) as ExamPeriodResponse[]);
  ...
  const submitCreateForm = async (model: Omit<ExamPeriodModel, 'id'>): Promise<void> => {
    let responseData = await postExamPeriod('', model);
  ...
  const submitUpdateForm = async (model: ExamPeriodModel): Promise<void> => {
    let responseData = await putExamPeriod(updateId, '', model);
    if (responseData.code != 200) {
      message.error(responseData.message);
      return;
    }
  ...
  const columns: ColumnsType<ExamPeriodResponse> = [
    {
      title: 'STT',
      dataIndex: 'index',
      width: 80,
    ...
    },
    {
      ...
      dataIndex: ['isCurrent'],
      render: (_, record) => <>{record.isCurrent ? <CheckOutlined /> : <CloseOutlined />}</>
    },
    {
      title: 'Thao tác',
      dataIndex: 'action',
            ...
            <Button type='link' onClick={async () => {
              Modal.confirm({
                ...
                onOk: async () => {
                  setLoadingDataState(true);
                  try {
                    let responseData: ResponseData = await deleteExamPeriod('', [record.id]);
              ...
              if (selectedRowDeleteKeys.length == 0) {
                ...
                Modal.confirm({
                  ...
                  onOk: async () => {
                    setLoadingDataState(true);
                    try {
                      let responseData: ResponseData = await deleteExamPeriod('', selectedRowDeleteKeys);
                      if (responseData.code != Code._200)
                        message.error(responseData.message);
                      else {
                        getData(pagination.current);
