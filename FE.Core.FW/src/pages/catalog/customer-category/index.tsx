...
import { deleteCustomerCategory, getCustomerCategory } from '@/apis/services/CustomerCategoryService';
function CustomerCategory() {
    ...
    const [list, setList] = useState<CustomerCategoryModel[]>([]);
    ...
    const deleteRecord = (id: string) => {
        Modal.confirm({
            ...
            cancelText: 'Hủy',
            onOk: async () => {
                const response = await deleteCustomerCategory(id);
    ...
    const onHandleShowModelCreate = async () => {
        navigate(`/catalog/customer-category/create`)
    };
    ...
    const searchFormSubmit = async (current: number = 1, pageSize: number = 20): Promise<void> => {
        try {
            ...
            const response: ResponseData = await getCustomerCategory(
                JSON.stringify(filter)
            );
            setList((response.data || []) as CustomerCategoryModel[]);
            setPagination({
                ...pagination,
                current,
    ...
    const columns: ProColumns<CustomerCategoryModel>[] = [
        {
            title: 'STT',
            dataIndex: 'index',
            width: 80,
            ...
            render: (_, record) => (
                <Space>
                    <Permission noNode navigation={layoutCode.catalogCustomerCategory as string} bitPermission={PermissionAction.Edit}>
                        <Button type="dashed" title='Cập nhật' loading={false} onClick={() => navigate(`/catalog/customer-category/edit/${record.id}`)}>
                            <EditOutlined />
                        </Button>
                    </Permission>
    ...
    return (
        <div className='layout-main-content'>
            ...
            >
                ...
                <ProTable<CustomerCategoryModel>
                    dataSource={list}
                    rowKey="id"
                    loading={loading}
                    pagination={{
                    ...
                    headerTitle="Tiêu đề"
                    toolBarRender={() => [
                        <Permission noNode navigation={layoutCode.catalogCustomerCategory as string} bitPermission={PermissionAction.Add}>
                            <Button htmlType='button' type='default' onClick={() => onHandleShowModelCreate()}>
                                <PlusOutlined />
                                Tạo mới
                            </Button>
