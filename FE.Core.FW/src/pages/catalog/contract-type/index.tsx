...
import { ContractTypeModel } from '@/apis/models/ContractTypeModel';
import { deleteContractType, getContractType } from '@/apis/services/ContractTypeService';
function ContractType() {
    ...
    const [list, setList] = useState<ContractTypeModel[]>([]);
    ...
    const onHandleShowModelCreate = async () => {
        navigate(`/catalog/contract-type/create`)
    };
    ...
    const searchFormSubmit = async (current: number = 1, pageSize: number = 20): Promise<void> => {
        try {
            ...
            const response: ResponseData = await getContractType(
                JSON.stringify(filter)
            );
            setList((response.data || []) as ContractTypeModel[]);
            setPagination({
                ...pagination,
                current,
    ...
    const columns: ProColumns<ContractTypeModel>[] = [
        {
            title: 'STT',
            dataIndex: 'index',
            width: 80,
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    <Permission noNode navigation={layoutCode.catalogCustomerCategory as string} bitPermission={PermissionAction.Edit}>
                        <Button type="dashed" title='Cập nhật' loading={false} onClick={() => navigate(`/catalog/contract-type/edit/${record.id}`)}>
                            <EditOutlined />
                        </Button>
                    </Permission>
                    <Permission noNode navigation={layoutCode.catalogCustomerCategory as string} bitPermission={PermissionAction.Delete}>
        ...
        <div className='layout-main-content'>
                        ...
                        <Row gutter={16} justify='start'>
                            <Col span={24} className='gutter-row' style={{ marginBottom: '8px' }}>
                                <Space>
                                    ...
                                    <Permission noNode navigation={layoutCode.catalogContractType as string} bitPermission={PermissionAction.Delete}>
            ...
            >
                ...
                <ProTable<ContractTypeModel>
                    ...
                    headerTitle="Tiêu đề"
                    toolBarRender={() => [
                        <Permission noNode navigation={layoutCode.catalogContractType as string} bitPermission={PermissionAction.Add}>
                            <Button htmlType='button' type='default' onClick={() => onHandleShowModelCreate()}>
                                <PlusOutlined />
                                Tạo mới
                            </Button>
